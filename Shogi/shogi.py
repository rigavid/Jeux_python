...
###########################################
from sty import bg as STY_BG, fg
...
save={}
eq={'R':'王','J':'玉','T':'飛','T+':'龍','F':'角','F+':'馬','O':'金','A':'銀','A+':'全','C':'桂','C+':'圭','L':'香','L+':'杏','P':'歩','P+':'と'}
...
def points_segment(p1, p2):
    ...
    if xa == xb:
        ...
        for i in xc:
            ...
            pos += 1
    return out
def dessine_koma(img:image,p1,p2,koma:str,c1=COL.white,c2=COL.blue,c3=COL.red,ep_l=1,l_t=2)->image:
    ori=0 if koma.isupper()else 180
    ct=ct_sg(p1,p2)
    ch=coosCircle(ct,tailles_koma[koma[0].upper()][0]/2,270+ori)
    cb=coosCircle(ct,tailles_koma[koma[0].upper()][0]/2,90+ori)
    cbd,cbg=(coosCircle(cb,tailles_koma[koma[0].upper()][1]/2,i+ori)for i in[0,180])
    chbg=coosCircle(cbg,tailles_koma[koma[0].upper()][0],ori-angles_koma[0])
    chhg=coosCircle(ch,tailles_koma[koma[0].upper()][1],angles_koma[3]+90+ori)
    chbd=coosCircle(cbd,tailles_koma[koma[0].upper()][0],ori+angles_koma[0]+180)
    chhd=coosCircle(ch,tailles_koma[koma[0].upper()][1],90-angles_koma[3]+ori)
    breyk = False
    ...
    cv2.fillPoly(img.img,[pts+round(tailles_koma[koma.upper()[0]][0]/27)],c2,lineType=cv2.LINE_AA)
    cv2.fillPoly(img.img,[pts],c1)
    cv2.polylines(img.img,[pts],True,c2,2,lineType=cv2.LINE_AA)
    a,b=5,1
    dtp=tailles_koma[koma[0].upper()][1]/11
    p1,p2=coosCircle(phg,dtp,ori),coosCircle(phd,dtp,180+ori)
    ch_y=p1[1]-phg[1]
    ...
    match koma.upper():
        ...
        case'P':
            ...
            LINES+=[[ptcg,pt_sg(ptcg,pthg,5,4)],[pt_sg(cth,chh),pt_sg(ptcd,pthd)],[chh,pt_sg(cth,chb)],[pt_sg(cth,ph3,5,2),pt_sg(ph3,cth,5,2)]]
            ...
            img.ellipse(ph1,(length,length),c2,ep_l,startAngle=55,endAngle=75,angle=ori,lineType=l_t)
            a,b=5,3
            ...
            c,d=1,3
            ptctb=ct_sg(ctb,cbb)
            LINES+=[[plg,pld],[pthlg,pt_sg(plg,pld,a,b)],[pthlg,pthld],[pt_sg(pt_sg(pb1,pb2,a,b),pt_sg(cbg,cbd,a,b),c,d),pt_sg(pt_sg(pb1,pb2,b,a),pt_sg(cbg,cbd,b,a),c,d)],[pt_sg(pt_sg(pb1,pb2,b,a),pt_sg(cbg,cbd,b,a),c,d),pt_sg(plg,pld,b,a)],[pt_sg(ptctb,pb3,a,b),pt_sg(ptctb,pb3,b,a)],[pt_sg(ptctb,pb4,a,b),pt_sg(ptctb,pb4,b,a)]]
            for a,b in LINES: img.line(a,b,c2,ep_l,l_t)
        case'P+':
...
def range2(start, stop, step):
    ...
    while True:
        a += step
        if a >= stop and step > 0: return(out)
        elif a <= stop and step < 0: return(out)
        out.append(a)
def mouse_get_case(event,x,y,flags,params)->None:
    if event==cv2.EVENT_LBUTTONDOWN:
        ...
        if clicked_in(pt,[Shogi.p1,Shogi.p4]):
            ...
            mouse.pos=pt
class Shogi:
    COL.bg=COL.new('#ff4013','rgb')
    fond=COL.new('#7896C8','rgb')
    COL.li=COL.new('#111728','rgb')
    proportions_sb=(33,36)
    proportions_sb_ext=(33.33,36.36)
    proportions_kd=12.12
    height_sb=(screen[1]-100)
    conversion=height_sb/proportions_sb[1]
    width_sb=conversion*proportions_sb[0]
    xa,xb=screen[0]/2-width_sb/2,screen[0]/2+width_sb/2
    ya,yb=screen[1]/2-height_sb/2,screen[1]/2+height_sb/2
    ...
    ctkda,ctkdb=(moyenne(xb+px,screen[0]),screen[1]/15*8),(moyenne(0,xa-px),screen[1]/15*7)
    mgkd=20
    save['ctkda']=ctkda
    save['ctkdb']=ctkdb
    save['dist']=square_root((conversion*(proportions_kd/2))**2*2)
    pkda=[coosCircle(save['ctkda'],save['dist'],90*i+45)for i in range(4)]
    pkdb=[coosCircle(save['ctkdb'],save['dist'],90*i+45)for i in range(4)]
    pkda=[pkda[2],pkda[3],pkda[1],pkda[0]]
    pkdb=[pkdb[2],pkdb[3],pkdb[1],pkdb[0]]
    ex,l_t=3,1
    img = new_img(RES.resolution,COL.bg)
    img.rectangle([p1[0]-px*ex,p1[1]-py*ex],[p4[0]+px*ex,p4[1]+py*ex],fond,0,l_t)
    img.rectangle(pkda[0],pkda[3],fond,0,l_t)
    img.rectangle(pkdb[0],pkdb[3],fond,0,l_t)
    for y in range2(ya,yb+1,dy):
    ...
    def __init__(self, tableau=defTab(), name:str='Shogi', j1:str='J1', j2:str='J2', fullscreen:bool=True) -> None:
        ...
        plateau = [[[[x,y],[x+self.dx,y+self.dy]]for x in range2(self.xa,self.xb,self.dx)]for y in range2(self.ya,self.yb,self.dy)]
        ...
        for i,y in enumerate(range(round(self.pkda[0][1]),round(self.pkda[2][1]),round(self.dy*coef))[:ny:]):#KOMADAI A
            ...
            for x in range(round(self.pkda[0][0]),round(self.pkda[1][0]),round(self.dx*coef))[:nx[i]:]:
        ...
        for i,y in enumerate(range(round(self.pkdb[0][1]),round(self.pkdb[2][1]),round(self.dy*coef))[:ny:]):#KOMADAI B
            ...
            for x in range(round(self.pkdb[0][0]),round(self.pkdb[1][0]),round(self.dx*coef))[:nx[i]:]:
    ...
    def image(self)->image:
        ind=0
        c1,c2,c3=COL.white,COL.black,COL.red
        ep_l,l_t=1,2
        ...
        for c in range(len(self.coos_komadai[0])):
            ...
            if n>0:
                dessine_koma(img,p1,p2,c,c1,c2,c3,ep_l,l_t)
            if n>1:
                sz=2
                pt=pt_sg(p1,p2,1,5)
                ckd1,ckd2=COL.black,COL.white
                img.write_centered(n,pt_sg(p1,p2,1,5),ckd1,ep_l*3,sz,cv2.FONT_HERSHEY_COMPLEX_SMALL,l_t)
                img.write_centered(n,[pt[0]-2,pt[1]+1],ckd2,ep_l,sz,cv2.FONT_HERSHEY_COMPLEX_SMALL,l_t)
            ind+=1
        ind=0
        for c in range(len(self.coos_komadai[1])):
            ...
            if n>0:
                dessine_koma(img,p1,p2,c.lower(),c1,c2,c3,ep_l,l_t)
            if n>1:
                sz=2
                pt=pt_sg(p1,p2,1,5)
                ckd1,ckd2=COL.black,COL.white
    ...
    def depl_piece(self,xo,yo,xa,ya):
        return((xo-xa,yo-ya)if self.trait else(xa-xo,ya-yo))
    ...
    def legal(self,xo,yo,xa,ya)->bool:
        ...
        match piece.upper():
            ...
            case'A':
                x,y=self.depl_piece(xo,yo,xa,ya)
                return(y==1 and abs(x)<=1)or(y==-1 and abs(x)==1)
            case'C':
                x,y=self.depl_piece(xo,yo,xa,ya)
                return y==2 and abs(x)==1
            case'P':
                x,y=self.depl_piece(xo,yo,xa,ya)
                return y==1 and x==0
            case'L':
    ...
    def move(self):
        ...
        ca=False
        while not ca:
            s_img=image(name=img.name,img=copy.deepcopy(img.img))
            s_img.fullscreen = img.fullscreen
            if xo>=0:s_img.rectangle(self.plateau[yo,xo,0],self.plateau[yo,xo,1],COL.lime,Shogi.ep_li)#Cadre de selection
            else:s_img.rectangle(self.coos_komadai[0 if self.trait else 1,yo,0],self.coos_komadai[0 if self.trait else 1,yo,1],COL.lime,Shogi.ep_li,l_t)#Cadre de selection
            for x in range(9):
                for y in range(9):
                    if self.legal(xo,yo,x,y):
                        c=self.plateau[y,x]
                        s_img.circle(ct_sg(c[0],c[1]),10,COL.violet,0,l_t)
    ...
    def ecran_fin(self):#TODO
        ...
        img.rectangle(a,b,COL.lime,0)
        img.rectangle(a,b,COL.red,Shogi.ep_li,Shogi.l_t)
        img.show()
    def start(self,out=False):
        img = image("Shogi").build()
        self.img = img
        if out:
            print(self)
        moves=[]
        while self.jouable() and img.is_opened():
            n=self.move()
            moves.append(n)
            if out:
                print(self)
