...
def dessine_kanji_soleil(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    cg, cd = ct_sg(p1, p3), ct_sg(p2, p4);lines = [ [p1, p3], [p1, p2], [p2, p4], [cg, cd], [p3, p4] ]
    for a, b in lines: img.ligne(a, b, c, ep, l_t)
def dessine_kanji_cereale(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ch, cb = ct_sg(p1, p2), ct_sg(p3, p4); ah, ab = ch, cb; pa = pt_sg(ah, ab, 3, 2)
    d1 = dist(p1, ch) * 0.6; d2 = dist(p1, ch) * 0.9; d3 = dist(p1, p3) * 0.8
...
def dessine_kanji_arbre(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ah, ab = ct_sg(p1, p2), ct_sg(p3, p4); pa = pt_sg(ah, ab, 3); d = dist(p1, ah) * 0.9
    d2 = dist(p1, p3) * 0.7; a1, a2 = coosCercle(pa, d, 180+ori), coosCercle(pa, d, ori)
...
def dessine_kanji_terre(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ch, cb = ct_sg(p1, p2), ct_sg(p3, p4); cg, cd = ct_sg(p1, p3), ct_sg(p2, p4)
    a, b = 17, 2; lines = [[pt_sg(cg, cd, a, b), pt_sg(cd, cg, a, b)], [pt_sg(ch, cb, a, b*2), cb], [p3, p4]]
    for a, b in lines: img.ligne(a, b, c, ep, l_t)
def dessine_kanji_cannellier(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ch, cb = ct_sg(p1, p2), ct_sg(p3, p4); ct, cd = ct_cr(p1, p2, p3, p4), ct_sg(p2, p4)
...
def dessine_kanji_or(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ch, cb = ct_sg(p1, p2), ct_sg(p3, p4); cg, cd = ct_sg(p1, p3), ct_sg(p2, p4); a = 20
    pcg, pcd = coosCercle(ch, dist(ch, p1), ori+180-a), coosCercle(ch, dist(ch, p1), ori+a)
    LINES = [[ch, pcg], [ch, pcd]]; a, b = 5, 3
    clh, clb = coosCercle(ch, tailles_koma['O'][1]/2/5, 90+ori), cb; ctl = ct_sg(clh, clb)
    pts = [clh, ctl, clb]; dts = [dist(p1, ch)*i for i in [0.8, 0.7, 1]]
    for i in range(3): LINES.append([coosCercle(pts[i], dts[i], ori), coosCercle(pts[i], dts[i], ori+180)])
    LINES.append([clh, clb]); pt1, pt2 = coosCercle(pts[1], dts[1], ori), coosCercle(pts[1], dts[1], ori+180)
...
def dessine_kanji_argent(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ...
    d = dist(p1, p2)/10; p1, p3 = coosCercle(ct_sg(p1, p2), d, ori), coosCercle(ct_sg(p3, p4), d, ori)
...
def dessine_kanji_promu(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ...
    ctb = ct_sg(ct, cb); img.ellipse(ctb, (dist(ctb, cd), dist(ctb, ch)*0.9), c, ep, l_t, 315, 335, -20+ori)
    for a, b in [[ctg, ctd], [peg, pfeg]]: img.ligne(a, b, c, ep, l_t)
...
def dessine_kanji_volant(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ch, cb, cg, cd = ct_sg(p1, p2), ct_sg(p3, p4), ct_sg(p1, p3), ct_sg(p2, p4); ct = ct_cr(p1, p2, p3, p4)
    a, b = 2, 3; an = -20; pt = ct_sg(ch, cg); d = dist(p1, p2)/6
...
def dessine_kanji_marcheur(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ch, cb, cg, cd = ct_sg(p1, p2), ct_sg(p3, p4), ct_sg(p1, p3), ct_sg(p2, p4); ct = ct_cr(p1, p2, p3, p4)
    an = -20; pt = ct_sg(cg, ct_sg(ch, ct)); ptb = ct_sg(p1, ct_sg(ch, ct)); d = dist(p1, p2)/6
    LINES = [[coosCercle(pt, d, an+ori), coosCercle(pt, d, an+180+ori)], [coosCercle(ptb, d, an+ori),
    coosCercle(ptb, d, an+180+ori)], [pt, ct_sg(p3, cb)], [pt_sg(ch, ct, 3), pt_sg(p2, cd, 3)],
...
def dessine_kanji_lune(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ch, cb, cg, cd = ct_sg(p1, p2), ct_sg(p3, p4), ct_sg(p1, p3), ct_sg(p2, p4); ct = ct_cr(p1, p2, p3, p4)
    a, b = 1, 2; LINES = [[p1, p3], [p1, p2], [p2, p4], [p4, pt_sg(p4, cb, 2)], [pt_sg(p1, cg, a, b),
...
def dessine_kanji_debout(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ch, cb, cg, cd = ct_sg(p1, p2), ct_sg(p3, p4), ct_sg(p1, p3), ct_sg(p2, p4); ct = ct_cr(p1, p2, p3, p4)
    col = c; a, b = 7, 2; c, d = 7, 4; cth = pt_sg(ch, ct, 3, 2)
    phg, phd = pt_sg(p1, cg, 3, 2), pt_sg(p2, cd, 3, 2); pbg, pbd = pt_sg(p3, cg, 3, 2), pt_sg(p4, cd, 3, 2)
...
def dessine_kanji_dragon_t(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ch, cb, cg, cd = ct_sg(p1, p2), ct_sg(p3, p4), ct_sg(p1, p3), ct_sg(p2, p4); ct = ct_cr(p1, p2, p3, p4)
    dessine_kanji_debout(img, p1, ch, cg, ct, c, ep, ori, l_t); dessine_kanji_lune(img, cg, ct, p3, cb, c, ep, ori, l_t)
    a,b = 4, 1; dessine_kanji_brutal(img, pt_sg(ch, p2, a, b), p2, pt_sg(cb, p4, a, b), p4, c, ep, ori, l_t)
...
def dessine_kanji_charriot(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ch, cb = ct_sg(p1, p2), ct_sg(p3, p4); pts = []
    LINES = []
    for i, y in enumerate(float_range(p1[1], p3[1], 6)):
        match i:
            case b if b in [i for i in range(2, 5)]:
                ...
                LINES.append([pt1, pt2]); pts.append([pt1, pt2])
            case a if a in [i for i in range(1, 6)]: img.ligne([p1[0], y], [p2[0], y], c, ep, l_t)
...
def dessine_kanji_cheval(img:image, p1, p2, p3, p4, c=col.black, ep=1, ori=0, l_t=2): ## DONE ##
    ch, cb, cg, cd = ct_sg(p1, p2), ct_sg(p3, p4), ct_sg(p1, p3), ct_sg(p2, p4); ct = ct_cr(p1, p2, p3, p4)
    ptbd, pttbd = pt_sg(p4, pt_sg(cb, ct, 2), 2), pt_sg(p4, cd, 4.5); ptcg, ptcd = pt_sg(cg, ct, 2), pt_sg(cd, ct, 2)
    ptsh = [ptcg, pt_sg(ptcg, ptcd, 2), pt_sg(ptcd, ptcg, 2), ptcd]; ptsb = [p3, pt_sg(p3, ptbd, 2), pt_sg(ptbd, p3, 2), ptbd]
    LINES = [ [p2, p1], [p1, cg], [cg, cd], [cd, pttbd], [pttbd, ptbd], [ch, ct], [pt_sg(p1, cg, 2),
    pt_sg(p2, cd, 2)], [pt_sg(cg, p1, 2), pt_sg(cd, p2, 2)]]; a, b = 5, 2
